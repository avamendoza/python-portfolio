...
def getDogSize(size):
    ...
    if size == "tiny":
        for i in range(0, len(dog_weights)):
            if int(dog_weights[i]) < 11:
                filtered_list.append(dog_breeds[i])
                print(filtered_list)
    if size == "small":
        for i in range(0, len(dog_weights)):
            if int(dog_weights[i]) < 26 and int(dog_weights[i]) > 9:
                filtered_list.append(dog_breeds[i])
                print(filtered_list)
    if size == "medium":
        for i in range(0, len(dog_weights)):
            if int(dog_weights[i]) < 61 and int(dog_weights[i]) > 25:
                filtered_list.append(dog_breeds[i])
                print(filtered_list)
    if size == "large":
        for i in range(0, len(dog_weights)):
            if int(dog_weights[i]) > 60:
                filtered_list.append(dog_breeds[i])
                print(filtered_list)


getDogSize(input("what size dog would you like"))
